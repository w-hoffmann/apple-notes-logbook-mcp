...
def assemble_body(summary: str, detail: str | None = None) -> str:
    ...
    Leading/trailing blank lines of ``detail`` are dropped so the heading is
    followed by exactly one blank line (the separator already provides it); a
    ``detail`` that is only whitespace/newlines yields a heading-only note.
    """
...
def consolidate(
    ...
    *,
    ...
    # Sort by an absolute epoch (float) rather than the datetime objects so a
    # mix of naive and tz-aware instants never raises a comparison TypeError.
    dated: list[tuple[float, date, str, RawNote]] = []
    for note in notes:
